...
(() => {
  ...
  function estilosHorarios(){
    if(document.getElementById('admin-horarios-barberos-estilos')) return;
    const s=document.createElement('style');
    s.id='admin-horarios-barberos-estilos';
    s.textContent=`
      .horarios-barberos-card{margin-top:20px}
      .horarios-barberos-ayuda{color:#999;font-size:13px;margin:-8px 0 20px;line-height:1.5}
      .horario-barbero-card{background:#101010;border:1px solid var(--borde);border-radius:12px;margin:0 0 18px;overflow:hidden}
      .horario-barbero-header{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:16px 18px;border-bottom:1px solid var(--borde);flex-wrap:wrap}
      .horario-barbero-header h4{margin:0;color:var(--principal);font-size:17px}
      .horario-barbero-estado{font-size:11px;padding:5px 9px;border-radius:20px;font-weight:700}
      .horario-barbero-estado.activo{background:#173317;color:#8be58b}
      .horario-barbero-estado.inactivo{background:#351717;color:#ff8888}
      .horarios-tabla{width:100%;border-collapse:collapse}
      .horarios-tabla th,.horarios-tabla td{padding:11px 10px;border-bottom:1px solid var(--borde);vertical-align:middle}
      .horarios-tabla th{color:var(--principal);font-size:11px;text-transform:uppercase}
      .horarios-tabla td{color:#ddd;font-size:13px}
      .horarios-tabla tr:last-child td{border-bottom:none}
      .horarios-tabla input[type="time"]{width:130px;padding:9px;border-radius:7px;border:1px solid #333;background:#0d0d0d;color:#fff}
      .horarios-tabla input[type="time"]:focus{border-color:var(--principal);outline:none}
      .horario-dia-activo{width:18px;height:18px;accent-color:var(--principal)}
      .horario-dia-descanso{color:#777;font-size:12px}
      .horario-guardar-barbero{margin:16px 18px 18px}
      .horarios-barberos-vacio{padding:20px;color:#888;text-align:center}
      @media(max-width:650px){
        .horarios-tabla{min-width:620px}
        .horarios-tabla-wrap{overflow-x:auto}
        .horario-barbero-header{align-items:flex-start}
      }
    `;
    document.head.appendChild(s);
  }
  ...
  async function obtenerBarberos(){
    const {data,error}=await C.from('barberos')
  ...
  async function obtenerHorarios(){
    const {data,error}=await C.from('barbero_horarios')
      .select('id,barbero_id,dia_semana,hora_inicio,hora_fin,activo')
      .in('barbero_id',(await obtenerBarberos()).map(b=>b.id));
    if(error) throw error;
    return data || [];
  }

  function htmlBarbero(barbero,horarios){
    const propios=horarios.filter(h=>String(h.barbero_id)===String(barbero.id));
    const filas=DIAS.map((dia,diaSemana)=>{
      const h=propios.find(x=>Number(x.dia_semana)===diaSemana);
      const activo=!!h?.activo;
      const inicio=String(h?.hora_inicio||'09:00').slice(0,5);
      const fin=String(h?.hora_fin||'18:00').slice(0,5);
      return `<tr>
        <td><strong>${dia}</strong></td>
        <td><input class="horario-dia-activo" type="checkbox" data-horario-activo="${barbero.id}-${diaSemana}" ${activo?'checked':''}></td>
        <td><input type="time" value="${esc(inicio)}" data-horario-inicio="${barbero.id}-${diaSemana}" ${activo?'':'disabled'}></td>
        <td><input type="time" value="${esc(fin)}" data-horario-fin="${barbero.id}-${diaSemana}" ${activo?'':'disabled'}></td>
        <td><span class="horario-dia-descanso" data-horario-estado="${barbero.id}-${diaSemana}">${activo?'Atiende':'Descanso'}</span></td>
      </tr>`;
    }).join('');

    return `<div class="horario-barbero-card" data-horario-barbero="${barbero.id}">
      <div class="horario-barbero-header">
        <h4>✂️ ${esc(barbero.nombre)}</h4>
        <span class="horario-barbero-estado ${barbero.activo?'activo':'inactivo'}">${barbero.activo?'BARBERO ACTIVO':'BARBERO INACTIVO'}</span>
      </div>
      <div class="horarios-tabla-wrap">
        <table class="horarios-tabla">
          <thead><tr><th>Día</th><th>Atiende</th><th>Desde</th><th>Hasta</th><th>Estado</th></tr></thead>
          <tbody>${filas}</tbody>
        </table>
      </div>
      <button type="button" class="btn horario-guardar-barbero" data-guardar-horarios="${barbero.id}">💾 Guardar horarios de ${esc(barbero.nombre)}</button>
    </div>`;
  }
  ...
  async function renderHorarios(){
    ...
    let card=document.getElementById('admin-horarios-barberos-card');
    if(!card){
      card=document.createElement('div');
      card.id='admin-horarios-barberos-card';
      card.className='card horarios-barberos-card';
      seccion.appendChild(card);
    }

    card.innerHTML='<h3>🕐 Horarios por barbero</h3><p class="horarios-barberos-ayuda">Cada barbero tiene su propia agenda. Configurá qué días atiende y su horario de inicio y fin. Estos horarios son los que utilizará la página de clientes para mostrar disponibilidad.</p><div class="horarios-barberos-vacio">Cargando horarios...</div>';

    try{
      const barberos=await obtenerBarberos();
      if(!barberos.length){
        card.querySelector('.horarios-barberos-vacio').textContent='Todavía no hay barberos cargados.';
        return;
      }

      const ids=barberos.map(b=>b.id);
      const {data:horarios,error}=await C.from('barbero_horarios')
        .select('id,barbero_id,dia_semana,hora_inicio,hora_fin,activo')
        .in('barbero_id',ids);
      if(error) throw error;

      card.innerHTML='<h3>🕐 Horarios por barbero</h3><p class="horarios-barberos-ayuda">Cada barbero tiene su propia agenda. Configurá qué días atiende y su horario de inicio y fin. Estos horarios son los que utilizará la página de clientes para mostrar disponibilidad.</p><div id="horarios-barberos-lista"></div>';
      const lista=card.querySelector('#horarios-barberos-lista');
      lista.innerHTML=barberos.map(b=>htmlBarbero(b,horarios||[])).join('');
      enlazarHorarios();
    }catch(e){
      console.error('Horarios por barbero:',e);
      card.innerHTML='<h3>🕐 Horarios por barbero</h3><p class="horarios-barberos-ayuda">Cada barbero tiene su propia agenda.</p><div class="mensaje error visible">No se pudieron cargar los horarios: '+esc(e.message)+'</div>';
    }
  }

  function enlazarHorarios(){
    document.querySelectorAll('[data-horario-activo]').forEach(chk=>{
      chk.addEventListener('change',()=>{
        const id=chk.dataset.horarioActivo;
        const inicio=document.querySelector(`[data-horario-inicio="${id}"]`);
        const fin=document.querySelector(`[data-horario-fin="${id}"]`);
        const estado=document.querySelector(`[data-horario-estado="${id}"]`);
        if(inicio)inicio.disabled=!chk.checked;
        if(fin)fin.disabled=!chk.checked;
        if(estado)estado.textContent=chk.checked?'Atiende':'Descanso';
      });
    });

    document.querySelectorAll('[data-guardar-horarios]').forEach(btn=>{
      btn.addEventListener('click',()=>guardarHorarios(btn.dataset.guardarHorarios,btn));
    });
  }

  async function guardarHorarios(barberoId,btn){
    const card=document.querySelector(`[data-horario-barbero="${barberoId}"]`);
    if(!card)return;

    const filas=[];
    for(let dia=0;dia<7;dia++){
      const id=`${barberoId}-${dia}`;
      const activo=!!card.querySelector(`[data-horario-activo="${id}"]`)?.checked;
      const inicio=card.querySelector(`[data-horario-inicio="${id}"]`)?.value;
      const fin=card.querySelector(`[data-horario-fin="${id}"]`)?.value;
      if(activo && (!inicio || !fin || inicio>=fin)){
        if(typeof mostrarGlobal==='function')mostrarGlobal(`Revisá el horario del ${DIAS[dia]} de ${card.querySelector('h4')?.textContent||'barbero'}.`,'error');
        return;
      }
      filas.push({dia,activo,inicio,fin});
    }
    ...
    try{
      const {data:existentes,error:errorLectura}=await C.from('barbero_horarios')
        .select('id,dia_semana')
        .eq('barbero_id',barberoId);
      if(errorLectura)throw errorLectura;

      for(const fila of filas){
        const existente=(existentes||[]).find(x=>Number(x.dia_semana)===fila.dia);
        if(existente){
          const payload={activo:fila.activo};
          if(fila.activo){payload.hora_inicio=fila.inicio;payload.hora_fin=fila.fin;}
          const {error}=await C.from('barbero_horarios').update(payload).eq('id',existente.id).eq('barbero_id',barberoId);
          if(error)throw error;
        }else if(fila.activo){
          const {error}=await C.from('barbero_horarios').insert({
            barbero_id:barberoId,
            dia_semana:fila.dia,
            hora_inicio:fila.inicio,
            hora_fin:fila.fin,
            activo:true
          });
          if(error)throw error;
      ...
      if(typeof mostrarGlobal==='function')mostrarGlobal('Horarios de '+(card.querySelector('h4')?.textContent||'barbero')+' guardados correctamente.');
      if(typeof cargarAgenda==='function')await cargarAgenda();
    }catch(e){
      console.error('Guardando horarios:',e);
      if(typeof mostrarGlobal==='function')mostrarGlobal('No se pudieron guardar los horarios: '+e.message,'error');
    }finally{
      btn.disabled=false;
  ...
  function iniciarHorarios(){
    renderHorarios();
    setTimeout(renderHorarios,700);
    const menu=document.querySelector('[data-seccion="barberos"]');
    if(menu)menu.addEventListener('click',()=>setTimeout(renderHorarios,80));
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',iniciarHorarios);else iniciarHorarios();
})();
